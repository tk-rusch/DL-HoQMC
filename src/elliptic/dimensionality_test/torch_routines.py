...
def train(ninp,lr,reg,width,depth,N,max_epochs,id):
    ninp = int(ninp)
    width = int(width)
    depth = int(depth)
    ...
    train_x1 = torch.from_numpy(np.loadtxt(data_path+'/dim_'+str(ninp)+'/In_'+str(int(np.log2(N)))+'.txt',delimiter=',').T)
    train_y1 = torch.from_numpy(np.loadtxt(data_path+'/dim_'+str(ninp)+'/Out'+str(int(np.log2(N)))+'.txt',skiprows=1)[:,1]).unsqueeze(-1)

    train_x2 = torch.from_numpy(np.loadtxt(data_path+'/dim_'+str(ninp)+'/In_'+str(int(np.log2(N)-1))+'.txt',delimiter=',').T)
    train_y2 = torch.from_numpy(np.loadtxt(data_path+'/dim_'+str(ninp)+'/Out'+str(int(np.log2(N)-1))+'.txt',skiprows=1)[:,1]).unsqueeze(-1)

    test_x = torch.from_numpy(np.loadtxt(data_path+'/dim_'+str(ninp)+'/In_13.txt',delimiter=',')[:,1:].T)
    test_y = torch.from_numpy(np.loadtxt(data_path+'/dim_'+str(ninp)+'/Out13.txt',skiprows=1)[1:,1]).unsqueeze(-1)
    ...
    Path('new_results'+str(ninp)).mkdir(parents=True, exist_ok=True)
    with open('new_results'+str(ninp)+'/N_'+str(int(N))+'.txt','a') as file:
