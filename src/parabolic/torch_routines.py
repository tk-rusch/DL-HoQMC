...
def train(lr,reg,width,depth,N,max_epochs,id):
    ...
    cwd = os.getcwd()
    data_path = cwd + '/../../data/parabolic'

    train_x1 = torch.from_numpy(np.loadtxt(data_path+'/In_'+str(int(np.log2(N)))+'.txt',delimiter=',').T)
    train_y1 = torch.from_numpy(np.loadtxt(data_path+'/Out'+str(int(np.log2(N)))+'.txt',skiprows=1)[:,1]).unsqueeze(-1)

    train_x2 = torch.from_numpy(np.loadtxt(data_path+'/In_'+str(int(np.log2(N)-1))+'.txt',delimiter=',').T)
    train_y2 = torch.from_numpy(np.loadtxt(data_path+'/Out'+str(int(np.log2(N)-1))+'.txt',skiprows=1)[:,1]).unsqueeze(-1)

    test_x = torch.from_numpy(np.loadtxt(data_path+'/In_12.txt',delimiter=',')[:,1:].T)
    test_y = torch.from_numpy(np.loadtxt(data_path+'/Out12.txt',skiprows=1)[1:,1]).unsqueeze(-1)
